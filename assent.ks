...
function adjust_pitch {
    ...
    until false {
        ...
        refreshline(2, 0, "Target Heading: " + initial_heading). refreshline(3, 0, "Prev/MaxT Rt: " + prev_thrust / maxthrust).
        wait 0.7.
    }.
}.
...
function init_assent {
    ...
    set prev_thrust to ship:availablethrust. SAS on. 
    STAGE. 
    wait 1.
    if (ship:availablethrust <= prev_thrust) { abort(). }.
    refreshline(0, 9, "Main engine ingintion.").
    wait 1. STAGE. 
    wait 0.5.
...
function execute_profile {
    ...
    from { local x is 0. } until x = moves step { set x to x + 3. } do {
        wait until altitude > profiles[x] and velocity:surface:mag > profiles[x+2]. refreshline(0, 9, "Starting Move " + (x / 3) + 1).
        if x <> 0 { set initial_heading to profiles[x-2]. }.
        adjust_pitch(initial_heading, profiles[x+1]). refreshline(0, 9, "Move " +  (x / 3) + 1 + " complete.").
    }.
}.
