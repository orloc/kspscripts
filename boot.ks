...
if addons:rt:hasconnection(ship) { 
    if hasfile(updatescript, 0) { 
        ...
        wait 0.5.
        if hasfile("update.ks", 1) { delete update.ks. }.
        rename updatescript to "update.ks". run update.ks. delete update.ks.
        print "File Loaded". wait 0.5.
    }.
}.
...
if hasfile("startup.ks", 0) { 
    download("startup.ks"). print "Startup Found and loaded".
    wait 0.5. run startup.ks.
} else { 
